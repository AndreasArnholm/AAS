...
class Scraper:
    ...
    def find_rows(self, table, last=None, report_name=None):
        while True:
        ...
        if last:
            while True:
                if rows[0].find_elements_by_tag_name('td')[6].text == last:
                    break
                self.driver.refresh()
                self.filter_report_name(report_name)
    ...
    def filter_report_name(self, report_name):
        ...
        self.enter_text_id('reportsfiles-filter-field-ReportName', report_name, submit=True)
    ...
    def run(self):
        ...
        # self.driver.find_element_by_id('reportsfiles-filter-count').find_element_by_xpath('..').click()
        # cb = '5gvinni-viewer'
        # WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.XPATH, '//button[contains(text(), "+ Filter")]'))).click()
        # self.id_click('ReportName')
        # self.enter_text_id('reportsfiles-filter-field-ReportName', report_name, submit=True)
        ...
        report_rows = self.find_rows('reportsfiles-table', last_report, report_name)
